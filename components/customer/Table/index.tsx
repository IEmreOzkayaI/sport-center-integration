...
export default function CustomerTable({ className, serverCustomers, limit }: { className?: string, serverCustomers?: any, limit: number }) {
    const {
        ...
        handleNext,
        handlePrev,
    } = useHook(serverCustomers, limit);

    return (
        <div className={cn("h-full grid grid-rows-12 shadow border rounded-md", className)}>
            <Card className="row-span-11 shadow-none border-none">
                <CardHeader className="px-7">
                    ...
                    <CardDescription className="text-center md:text-left">
                        Salon ve Diyetisyen İş Birliğine ait Üyeleri Buradan Görebilirsin
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <Table>
